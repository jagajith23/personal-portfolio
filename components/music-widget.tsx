...
export default function MusicWidget() {
        ...
        const fetchMusic = async () => {
            ...
            try {
                ...
                if (validSongs.length === 0)
                    throw new Error("No valid previews found");
    ...
    const handleAudioError = () => {
        ...
        console.warn(
            `Error loading ${currentSong?.title}. Skipping to next...`,
        );
    ...
    if (loading) {
        return (
            <div className="w-full max-w-xs h-24 p-4 rounded-xl border border-white/[0.08] bg-white/[0.02] flex items-center justify-center">
    ...
    return (
        <div className="w-full max-w-xs p-4 rounded-xl border border-white/[0.08] bg-white/[0.02] backdrop-blur-sm relative overflow-hidden font-aoboshi group">
            <audio
                ref={audioRef}
    ...
    return (
        <div className="flex items-end gap-[2px] h-3">
            {[0, 1, 2, 3].map((bar) => (
                <motion.div
                    key={bar}
                    className="w-[2px] bg-signature rounded-t-sm"
                    animate={{
                        height: [4, 12, 4],
                    }}
                    transition={{
